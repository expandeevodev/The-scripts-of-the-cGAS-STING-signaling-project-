...
my $genome_index="mouse_genome_index"; # the mouse genomes indexed using Hisat2
opendir FH,$link;
...
foreach my $sample(sort keys %sample)
{
        ...
        my $out_sam_file=$out.".sam";
         my $file1=$out."_1.fq.gz"; #Rna2_R1.fq.gz
         my $file2=$out."_2.fq.gz";
         my $file1_loci=$link."/".$sample."/".$file1;
         my $file2_loci=$link."/".$sample."/".$file2;
       
        
        system("hisat2 -p 40  -dta -k 5 -x $genome_index -1 $file1_loci -2 $file2_loci -S $out_sam_file");
